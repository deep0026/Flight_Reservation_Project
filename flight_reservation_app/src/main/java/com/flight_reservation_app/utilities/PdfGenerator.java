...
public class PdfGenerator {
    ...
    public  void generatePDF(String filePath, String name, String emailId, String phone, String operatingAirlines, Date departureDate, String departureCity, String arrivalCity) {
        try {
            ...
            //supply the document object reference here
            PdfWriter.getInstance(document, new FileOutputStream(filePath));//output stream writes the content into the file
            document.open();//here we are openiing document 
            addMetaData(document);//once opening we are adding meta data into it
    ...
    private static void addTitleAndTable(Document document, String name, String emailId, String phone, String operatingAirlines, String departureDate, String departureCity, String arrivalCity)
        ...
        Paragraph preface = new Paragraph(); //adding paragraph
        
        
        preface.add(new Paragraph("Flight Booking Details", catFont));//heading

       
        preface.add(new Paragraph(
                "Report generated by: " + "Flight Reservation Company " + new Date(), //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                smallBold));//when the tickets are generated current date is also added
       
        document.add(preface);  
        document.add( Chunk.NEWLINE );//moving to newline
        document.add( Chunk.NEWLINE );
        
        
        PdfPTable table = new PdfPTable(2); //this is creating the table with 2 columns
        ...
        PdfPCell c1 = new PdfPCell(new Phrase("Passenger Detail"));//new passenger Detail cell is added
        c1.setHorizontalAlignment(Element.ALIGN_CENTER);
        c1.setColspan(2);//merge
        table.addCell(c1);//cell1 data is added
        ...
        PdfPTable table1 = new PdfPTable(2);//again create a table
        table1.setWidthPercentage(100);//set width
        // t.setBorderColor(BaseColor.GRAY);
        // t.setPadding(4);
        // t.setSpacing(4);
        // t.setBorderWidth(1);

        PdfPCell c2 = new PdfPCell(new Phrase("Flight Detail"));//create cell
